...
import { Text,View,StyleSheet,ImageBackground,StatusBar,SafeAreaView,Platform,Alert,Image,FlatList,Dimensions } from "react-native";

export default class Meteoro extends React.Component {
    constructor(props) {
        super(props)
        this.state = { meteors: {} }
    }
    ...
    getMeteors = () => {
        axios.get("https://api.nasa.gov/neo/rest/v1/feed?api_key=cTTmzSDFLM3IwsfKg2JHfIvLvnWgce0UgbsWuddq")
            .then((response) => {
    ...
    render() {
        if (Object.keys(this.state.meteors).length === 0) {
            ...
            var meteorMatrix=Object.keys(this.state.meteors).map(meteorDate=>{
                return this.state.meteors[meteorDate]
            })

            let meteors = [].concat.apply([],meteorMatrix)
            ...
            meteors.sort((a,b)=> {return a.threat_score-b.threat_score})
